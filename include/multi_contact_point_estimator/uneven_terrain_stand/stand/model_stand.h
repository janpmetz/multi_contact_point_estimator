...
public:
	ModelStand();
	virtual ~ModelStand();
	FootStateUneven tensorflow_predict(std::vector<vec3> const &points, vec3 zmpv, int dataWidth, int dataHeight, MultiContactPointModel* const &model, double yaw,vigir_terrain_classifier::HeightGridMap::Ptr height_grid_map, FootForm ff, Leg leg);
private:
